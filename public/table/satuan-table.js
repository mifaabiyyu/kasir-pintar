...
var SatuanDataTable = (function () {
    ...
    var initDatatable = function () {
        oTable = $("#satuan_table").DataTable({
            processing: true,
            serverSide: true,
            retrieve: true,
            ajax: {
                data: (d) => Object.assign(d, filterData),
                url: route("satuan-data.index"),
            },
            columns: [
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        <div class="">
                            ...
                            <a  id="edit-data" data-id="${row.id}" data-bs-toggle="modal" data-bs-target="#editSatuanModal" class="btn btn-icon btn-bg-light btn-primary btn-active-color-secondary btn-sm me-1">
    ...
    var handleDeleteRows = () => {
        ...
        deleteButtons.forEach((d) => {
            ...
            d.addEventListener("click", function (e) {
                ...
                // Get customer name
                const satuan = parent.querySelectorAll("td")[1].innerText;
                var kode = $(this).data("id");
                Swal.fire({
                    text: "Are you sure you want to delete " + satuan + "?",
                    icon: "warning",
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "DELETE",
                            url: route("satuan-data.destroy", kode),
                            data: {
                                _token: csrf,
                            },
                            success: function (response) {
                                Swal.fire({
                                    text: "You have deleted " + satuan + "!.",
                                    icon: "success",
                                    buttonsStyling: false,
    ...
    //         '[data-kt-customers-table-filter="reset"]'
    //     );
